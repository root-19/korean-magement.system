...
@section('subheading', 'Enrol a student and assign their instructor')
...
@section('content')
    <form method="POST" action="{{ route('admin.students.store') }}" class="grid gap-4 lg:grid-cols-3">
        @csrf
                            ...
                            @foreach ([
                                ...
                                <label class="flex cursor-pointer items-start gap-2.5 rounded-lg border px-3 py-2.5 transition"
                                    ...
                                    <input type="radio"
                                           ...
                                           @checked((string) $value === old('is_regular'))
        ...
        <div class="space-y-4">
            <x-card title="Assigned instructor">
                <select name="instructor_id" class="form-select" aria-label="Instructor">
                    ...
                    @foreach ($instructors as $instructor)
                        <option value="{{ $instructor->id }}"
                            @selected((int) old('instructor_id') === $instructor->id)>
                            {{ $instructor->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <p class="mt-3 text-xs text-gray-500">
                    Leave unassigned to park the student until an instructor is free —
                    they appear under the Unassigned filter until then.
                </p>
            </x-card>

            <x-card title="What happens next">
                <p class="text-sm text-gray-300">
                    An admin enrolment is approved on save: there is nobody left to
                    approve it. The student is teachable and billable straight away.
                </p>
